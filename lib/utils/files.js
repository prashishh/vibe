...
async function createQuickReference(projectPath) {
  let content = `# Vibe Framework - ${path.basename(projectPath)}\n\n`;
  content += `## Quick Start\n\n`;
  content += `The Vibe Framework helps you build software with AI assistance while maintaining quality through guards.\n\n`;
  content += `---\n\n`;
  content += `## Workflow\n\n`;
...
async function createProjectMemory(projectPath, ide, skillsDir) {
  const content = `# Vibe Framework Project

This project uses the Vibe Framework for AI-assisted software delivery.
