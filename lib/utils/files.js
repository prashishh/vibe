...
async function copySkills(frameworkPath, projectPath, ide, installMethod = 'copy') {
  // Select adapter source: Claude gets its specific adapter, Codex gets its own,
  // all others get the generic adapter (no agent-specific frontmatter).
  const adapterMap = {
    claude: 'adapters/claude/commands',
    codex: 'adapters/codex/commands'
  };
  const adapterDir = adapterMap[ide] || 'adapters/generic/commands';
  const skillsSourceDir = path.join(frameworkPath, adapterDir);
  const installedDirs = [];
